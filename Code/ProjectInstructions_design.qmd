...
```{r organism boxplot}
# D-values by Organism
ggplot(hpp, aes(x = Organism, y = Response, fill = Organism)) +
  geom_boxplot() +
...
ggplot(hpp, aes(x = Pressure, y = Response, fill = Pressure)) +
  geom_boxplot() +
...
```{r pH boxplot}
# D-values by pH
ggplot(hpp, aes(x = pH, y = Response, fill = pH)) +
  geom_boxplot() +
...
```{r Acid boxplot}
# D-values by Acid
ggplot(hpp, aes(x = Acid, y = Response, fill = Acid)) +
  geom_boxplot() +
...
# Model fitting
hpp_mod <- lm(Response ~ Organism + Pressure + pH + Acid, data = hpp)
...
```{r Anova}
Anova(hpp_mod, type = 3)

```

```{r}
hpp_resid <- augment(hpp_mod) %>%
  mutate(.studresid=rstudent(hpp_mod))
...
```{r}
ggplot(hpp_resid, aes(x=.fitted, y=.studresid))+
...
```{r}
ggplot(hpp_resid, aes(sample=.studresid))+
  stat_qq(  shape = 21,
            fill = "purple", 
            size = 3,
            alpha = .7
...
```{r}
ggplot(hpp_resid, aes(x=.studresid))+
...
hpp_means <- emmeans(hpp_mod, ~ Pressure : Acid : pH )
...
ggplot(resid_fitted_df, aes(x = Fitted, y = Residuals)) +
...
anova_model <- aov(Response ~ Pressure * pH * Acid * Organism, data = data)

emm_all <- bind_rows(
  as.data.frame(emmeans(model, ~ Pressure)) %>% mutate(Factor = "Pressure", Level = as.character(Pressure)),
  as.data.frame(emmeans(anova_model, ~ pH)) %>% mutate(Factor = "pH", Level = as.character(pH)),
  as.data.frame(emmeans(anova_model, ~ Acid)) %>% mutate(Factor = "Acid", Level = as.character(Acid)),
  as.data.frame(emmeans(anova_model, ~ Organism)) %>% mutate(Factor = "Organism", Level = as.character(Organism))
)

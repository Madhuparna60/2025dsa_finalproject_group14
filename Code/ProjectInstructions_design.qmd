...
```{r}
# Boxplot of D-values by each factor
ggplot(Data, aes(x = Organism, y = Response, fill = Organism)) +
  geom_boxplot() +
  facet_grid(Acid ~ Pressure + pH) +
  theme_minimal() +
  labs(title = "Distribution of D-values by Organism, pH, Pressure, and Acid")

# Check interaction effects visually
interaction.plot(Data$Pressure, Data$Organism, Data$Response,
                 col = c("red", "blue"), lwd = 2,
                 xlab = "Pressure", ylab = "Response", trace.label = "Organism")
...
model <- aov(Response ~ Organism * Pressure * pH * Acid, data = Data)
summary(model)

# If Type III SS is required
model_lm <- lm(Response ~ Organism * Pressure * pH * Acid, data = Data)

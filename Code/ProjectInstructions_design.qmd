...
ggplot(data_long, aes(x = Level, y = Response, colour = Level)) +
  geom_boxplot() +
...
# Model fitting
mod1_hpp <- lmer(Response ~ Pressure + pH + Acid + Organism + (1 | Rep), data = hpp)
...
#Residual Extraction 
```{r}
hpp_resid <- augment(mod1_hpp) %>%
  mutate(.stdresid = resid(mod1_hpp,
                             type = "pearson",
                             scaled = T
                              )) %>%
  left_join(hpp)
...
mod1_hpp_randeff <- ranef(mod1_hpp)[[1]] 

ggplot(mod1_hpp_randeff, 
...
# Model fitting
mod2_hpp <- lmer(yield_mgha ~ (1|rep) + sr_ksha,
                 data = reg_dfw
                 )
...
#Residual Outliers
```{r}
ggplot(hpp_resid, aes(x=.fitted, y=.stdresid))+
...
##(h) Model Means and Pairwise comparison
```{r means}
emmeans_model <- emmeans(hpp_reg, ~ Pressure*pH*Acid | Organism)
summary(emmeans_model)

pairs(emmeans_model)

ggplot(hpp, aes(x = Acid, y = Response, fill = Organism)) +
  facet_grid(~ Pressure) +
  geom_boxplot() +
  stat_summary(fun = mean, geom = "point", shape = 20, size = 3, color = "red") +
  labs(title = "Effect of Acid and Organism on Response", x = "Acid", y = "Response") +
  theme_minimal() +
  theme(

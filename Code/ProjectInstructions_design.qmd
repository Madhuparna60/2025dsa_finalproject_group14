...
```{r Pressure boxplot}
# D-values by Pressure and Organism
ggplot(hpp, aes(x = factor(Pressure), y = Response, colour = factor(Pressure))) +
  ...
  labs(
    x = "Pressure (MPa)",
    y = "D-value",
    title = "D-value by Pressure and Organism"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```
...
```{r pH boxplot}
# D-values by pH
ggplot(hpp, aes(x = factor(pH), y = Response, colour = factor(pH))) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  facet_wrap(~ Organism) +
  labs(
    x = "pH",
    y = "D-value",
    title = "D-value by pH and Organism"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```
...
```{r Acid boxplot}
# D-values by Acid
ggplot(hpp, aes(x = factor(Acid), y = Response, colour = factor(Acid))) +
  geom_boxplot(outlier.shape = NA, alpha = 0.5) +
  geom_jitter(width = 0.2, alpha = 0.6) +
  facet_wrap(~ Organism) +
  labs(
    x = "Acid",
    y = "D-value",
    title = "D-value by Acid and Organism"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```
...
# Model fitting
hpp_rcbd <- lm(Response ~  Organism * Pressure * pH * Acid,
             data = hpp)
...
```{r Anova}
Anova(hpp_rcbd, type = 3)

```
##(g) Linear model assumptions
#Residual Extraction 
```{r}
hpp_resid <- augment(hpp_rcbd) %>%
  mutate(.studresid=rstudent(hpp_rcbd))
...
res_data <- data.frame(
  Fitted = fitted(hpp_rcbd),
  Residuals = resid(hpp_rcbd)
)
```
...
ggsave("../Output/Residualfitted.png",
       height = 4,
       width = 6)
```
...
```{r means}
hpp_means <- emmeans(hpp_rcbd, ~Pressure * pH * Acid * Organism)
```
```{r pc}

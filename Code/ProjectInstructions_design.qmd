...
The d-values ranging from 0 to 15.221 seconds Mg/ha.  
...
ggplot(hpp, aes(x = factor(pH), y = Response, colour = Organism)) +
geom_boxplot() +
labs(title = "pH vs. D-value", x = "pH", y = "D-value") +
theme_minimal()


ggplot(hpp, aes(x = factor(Acid), y = Response, colour = Organism)) +
geom_boxplot() +
labs(title = "Acid vs. D-value", x = "Acid", y = "D-value") +
theme_minimal()


ggplot(hpp, aes(x = factor(Pressure), y = Response, colour = Organism)) +
geom_boxplot() +
labs(title = "Pressure vs. D-value", x = "Pressure", y = "D-value") +
theme_minimal()
...
```{r interaction boxplot}
# Create interaction plot
ggplot(hpp, aes(x = factor(Pressure), y = Response, colour = Organism)) +
  geom_boxplot() +
  facet_grid(Acid ~ pH) +
  labs(title = "Interaction Plot: Pressure, Acid, pH vs. D-value", x = "Pressure", y = "D-value") +
...
mod1_hpp <- lm(Response ~ Pressure + pH + Organism + Acid + Rep, data = hpp)
summary(mod1_hpp)

```

```{r mod2}
# Fit GLM (assuming Poisson distribution for example)
mod2_hpp <- lm(Response ~ Pressure + pH + Organism + Acid, data = hpp, family = poisson())
summary(mod2_hpp)
...
mod3_hpp <- lmer(Response ~ Pressure + Acid + Organism + pH + (1 | Rep), data = hpp)
summary(mod3_hpp)

```
##(f) Assumptions Checking
```{r residual}
#Residual Checks for Linear Model
# Residual plots
...
# QQ plot
qqnorm(residuals(mod1_hpp))
qqline(residuals(mod1_hpp))

```

```{r residual glm}
#Residual Checks for General Linear Model
# Residual plots for GLM
par(mar = c(2, 2, 2, 2))
plot(mod2_hpp)

# QQ plot
qqnorm(residuals(mod2_hpp))
qqline(residuals(mod2_hpp))

```
```{r}
#Residual Checks for Mixed Effect model
#Residual Plots
...
# QQ plot
qqnorm(residuals(mod3_hpp))
qqline(residuals(mod3_hpp))
```
...
emmeans_mod1_hpp <- emmeans(mod1_hpp, ~ Acid + Organism)
...
# Estimated marginal means
emmeans_mod2_hpp <- emmeans(mod2_hpp, ~ Acid + Organism)
...
# Estimated marginal means
emmeans_mod3_hpp <- emmeans(mod3_hpp, ~ Acid + Organism)
...
aic_model1 <- AIC(mod1_hpp)
aic_model2 <- AIC(mod2_hpp)
aic_model3 <- AIC(mod3_hpp)
...
best_model <- list(mod1_hpp, mod2_hpp, mod3_hpp)[[which.min(c(aic_model1, aic_model2, aic_model3))]]
summary(best_model)
...
qqPlot(best_model$residuals, main = "QQ Plot for Best Model")
...
ggplot(hpp, aes(x = Pressure, y = Response, fill = Organism)) +
geom_boxplot() +
 facet_grid(Acid ~ pH) +
theme_bw() +
theme(
...
strip.text = element_text(size = 7),
 ...
 legend.text = element_text(size = 5)
  ...
  scale_x_continuous(
limits = c(500, 700), # Set the limits for the x-axis
breaks = seq(500, 700, by = 100) # Set the breaks for the x-axis
) +
labs(
 title = "Interaction Plot: Pressure, Acid, pH vs. Response",
x = "Pressure",
y = "Response",
color = "Organism"
)
